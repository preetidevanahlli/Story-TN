      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Open a prompt to purchase or view generated content</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {prompts.length === 0 && <div className="text-muted-foreground">No prompts yet.</div>}
            {prompts.map((p) => (
              <div key={p.id} className="flex items-center justify-between border rounded p-3">
                <div>
                  <div className="font-medium">{new Date(p.createdAt).toLocaleString()}</div>
                  <div className="text-sm text-muted-foreground">Purchases: {p.purchases.map(x => x.product).join(", ") || "None"}</div>
                </div>
